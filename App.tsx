  ...
  useEffect(() => {
    ...
    const handleReconnect = () => {
      ...
      reconnectTimerRef.current = window.setTimeout(() => {
        ...
        audioRef.current.play().catch(() => handleReconnect());
      }, RECONNECT_DELAY);
    };
    ...
    if (isPlaying) {
      ...
      audio.addEventListener('stalled', handleReconnect);
    ...
    return () => {
      ...
      audio.removeEventListener('stalled', handleReconnect);
    ...
    <div className="relative min-h-[100dvh] flex flex-col items-center justify-between overflow-x-hidden px-6 bg-black">
      ...
      <div className="absolute top-4 md:top-6 right-6 md:right-10 z-20">
        <div className="flex items-center gap-2 text-[10px] md:text-xs uppercase tracking-[0.15em] font-medium">
          <button onClick={() => setLang('mk')} className={`transition-all duration-300 ${lang === 'mk' ? 'text-white font-bold px-0.5' : 'text-stone-500 hover:text-stone-300'}`}>MK</button>
          <span className="text-white/10 select-none">|</span>
          <button onClick={() => setLang('en')} className={`transition-all duration-300 ${lang === 'en' ? 'text-white font-bold px-0.5' : 'text-stone-500 hover:text-stone-300'}`}>EN</button>
          <span className="text-white/10 select-none">|</span>
          <button onClick={() => setLang('ru')} className={`transition-all duration-300 ${lang === 'ru' ? 'text-white font-bold px-0.5' : 'text-stone-500 hover:text-stone-300'}`}>RU</button>
        </div>
      </div>
